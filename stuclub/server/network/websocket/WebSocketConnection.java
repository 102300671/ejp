...
public class WebSocketConnection {
    ...
    public WebSocketConnection(WebSocket conn, MessageRouter messageRouter, WebSocketServer webSocketServer) {
        ...
        this.clubDAO = new ClubDAO(messageRouter);
    ...
    public void onClose(int code, String reason, boolean remote) {
        ...
        if (isAuthenticated && currentUser != null) {
            ...
            try (Connection connection = dbManager.getConnection()) {
                userDAO.updateUserStatusWithLogoutTime(currentUser.getId(), "INACTIVE", connection);
                System.out.println("用户状态已更新为INACTIVE: " + currentUser.getUsername());
            ...
            switch (message.getType()) {
                ...
                case TEXT:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        messageDAO.saveMessage(storageMessage, "CLUB".equals(conversationType) ? "CLUB" : "PRIVATE", conversationId, connection);
                ...
                case JOIN:
                    ...
                    if (joinConversationId != null) {
                        for (String rId : messageRouter.getClubs().keySet()) {
                            Club room = messageRouter.getClubs().get(rId);
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        if (conversation == null) {
                            // 创建新的conversation
                            int newConversationId = conversationDAO.createConversation("CLUB", roomName, connection);
                        ...
                        // 检查用户是否已在房间中（数据库层面）
                        boolean alreadyInRoom = clubDAO.isUserInClub(roomId, userId, connection);
                        if (alreadyInRoom) {
                        ...
                        // 加入房间（内存层面）
                        messageRouter.joinClub(userId, roomId);
                        
                        // 只在用户不在房间时才将用户加入到room_member表
                        if (!alreadyInRoom) {
                            clubDAO.joinClub(roomId, userId, connection);
                        }
                        ...
                        responseData.addProperty("type", "CLUB");
                ...
                case LEAVE:
                    ...
                    // 如果没有通过conversationId找到，尝试通过消息内容中的房间名称查找
                    if (leaveRoomId == null && message.getContent() != null) {
                        String roomNameFromContent = message.getContent();
                        try {
                            ...
                            if (json.has("room_name")) {
                                roomNameFromContent = json.get("room_name").getAsString();
                            }
                        } catch (Exception e) {
                        }
                        ...
                        for (String rId : messageRouter.getClubs().keySet()) {
                            Club room = messageRouter.getClubs().get(rId);
                            if (room.getName() != null && room.getName().equals(roomNameFromContent)) {
                                leaveRoomId = rId;
                                leaveRoomName = room.getName();
                                leaveConversationId = room.getConversationId();
                                break;
                            }
                        }
                    }
                    
                    System.out.println("处理离开社团消息: " + currentUser.getUsername() + "离开" + leaveRoomName + "社团");
                    
                    // 将用户离开目标社团
                    if (leaveRoomId != null) {
                        // 离开社团
                        String userId = String.valueOf(currentUser.getId());
                        messageRouter.leaveClub(userId, leaveRoomId);
                        ...
                        // 广播离开消息
                        messageRouter.broadcastToClub(leaveRoomId, messageCodec.encode(leaveMessage));
                    }
                    break;
                ...
                case CREATE_ROOM:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        try {
                            // 创建新的conversation
                            int newConversationId = conversationDAO.createConversation("CLUB", createRoomName, connection);
                            ...
                            if ("PRIVATE".equals(roomType)) {
                                newRoom = new PrivateClub(createRoomName, null, messageRouter);
                                clubDAO.insertPrivateClub((PrivateClub) newRoom, connection);
                            } else {
                                newRoom = new PublicClub(createRoomName, null, messageRouter);
                                clubDAO.insertPublicClub((PublicClub) newRoom, connection);
                            }
                            ...
                            // 添加房间到消息路由器
                            messageRouter.addClub(newRoom);
                            
                            // 将创建者作为房主加入房间
                            clubDAO.joinClub(newRoom.getId(), ownerId, "OWNER", connection);
                            messageRouter.joinClub(ownerId, newRoom.getId());
                ...
                case EXIT_ROOM:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        // 从消息路由器中移除用户
                        messageRouter.leaveClub(userId, exitRoomId);
                        
                        // 从数据库中删除room_member记录
                        clubDAO.leaveClub(exitRoomId, userId, connection);
                        ...
                        // 广播退出消息
                        messageRouter.broadcastToClub(exitRoomId, messageCodec.encode(exitMessage));
                ...
                case LIST_ROOMS:
                    ...
                    System.out.println("处理社团列表请求: " + currentUser.getUsername());
                    try (Connection connection = dbManager.getConnection()) {
                        String sql = "SELECT c.name, c.category as type, conv.id as conversation_id FROM club c JOIN club_member cm ON c.id = cm.club_id LEFT JOIN conversation conv ON conv.club_id = c.id AND conv.type = 'CLUB' WHERE cm.user_id = ? AND cm.status = 'APPROVED' AND c.status = 'APPROVED'";
                        try (java.sql.PreparedStatement pstmt = connection.prepareStatement(sql)) {
                            ...
                            try (java.sql.ResultSet rs = pstmt.executeQuery()) {
                                ...
                                while (rs.next()) {
                                    ...
                                    roomObj.addProperty("name", rs.getString("name"));
                                    roomObj.addProperty("type", rs.getString("type"));
                        ...
                        System.err.println("获取社团列表失败: " + e.getMessage());
                        e.printStackTrace();
                        Message systemMessage = new Message(MessageType.SYSTEM, "server", "获取社团列表失败: " + e.getMessage(), null);
                        send(messageCodec.encode(systemMessage));
                    }
                    break;
                ...
                case LIST_ROOM_USERS:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        System.out.println("当前房间列表: " + messageRouter.getClubs().keySet());
                        
                        // Find room by conversation_id
                        if (listUsersConversationId != null) {
                            for (String rId : messageRouter.getClubs().keySet()) {
                                Club r = messageRouter.getClubs().get(rId);
                        ...
                        // 根据 conversation_id 获取 club_id
                        int clubId = -1;
                        String clubSql = "SELECT club_id FROM conversation WHERE id = ? AND type = 'CLUB'";
                        try (PreparedStatement pstmt = connection.prepareStatement(clubSql)) {
                            pstmt.setInt(1, listUsersConversationId);
                        ...
                        if (clubId <= 0) {
                            Message systemMessage = new Message(MessageType.SYSTEM, "server", "获取房间用户列表失败: 社团不存在", null);
                            send(messageCodec.encode(systemMessage));
                            break;
                        }
                        
                        // 获取社团用户列表（包含角色信息）
                        List<Map<String, Object>> usersList = clubDAO.getClubMembersWithRoles(String.valueOf(clubId), connection);
                        
                        // 获取在线用户ID集合
                        List<Map<String, Object>> roomUsersList = messageRouter.getClubUsers(String.valueOf(clubId));
                        ...
                        response.append("],\"currentUserRole\":\"").append(clubDAO.getUserRole(String.valueOf(clubId), String.valueOf(currentUser.getId()), connection)).append("\"");
                        
                        // 添加房主和管理员信息
                        response.append(",\"ownerId\":\"").append(clubDAO.getCreatorId(String.valueOf(clubId), connection)).append("\"");
                        response.append(",\"adminIds\":[");
                        boolean firstAdmin = true;
                        for (String adminId : clubDAO.getAdminIds(String.valueOf(clubId), connection)) {
                            if (!firstAdmin) {
                                response.append(",");
                            }
    ...
    private void handleRegister(Message message) {
        try (Connection connection = dbManager.getConnection()) {
            ...
            if (realName == null || realName.trim().isEmpty()) {
                sendAuthFailure("请填写真实姓名");
                return;
            }
            
            if (studentId == null || studentId.trim().isEmpty()) {
                sendAuthFailure(role.equals("TEACHER") ? "请填写工号" : "请填写学号");
                return;
            }
            ...
            if (userDAO.isStudentIdExists(studentId, connection)) {
                sendAuthFailure("学号/工号已被注册");
                return;
            }
            
            User newUser = new User(0, username, password, null, null, role, realName, studentId);
            ...
            System.out.println("用户注册成功: " + username + " (ID: " + userId + ", 角色: " + (role != null ? role : "STUDENT") + ")");
            
            // 将用户加入system房间
            try {
                // 查找system房间
                Club systemRoom = null;
                String systemRoomId = null;
                for (String roomId : messageRouter.getClubs().keySet()) {
                    Club room = messageRouter.getClubs().get(roomId);
                ...
                if (systemRoom != null) {
                    // 将用户加入system房间
                    clubDAO.joinClub(systemRoomId, String.valueOf(userId), "MEMBER", connection);
    ...
    private void handleLogin(Message message) {
        try (Connection connection = dbManager.getConnection()) {
            ...
            if (!userDAO.validateUserByStudentId(studentId, password, connection)) {
                sendAuthFailure("学号/工号或密码错误");
                return;
            }
            
            currentUser = userDAO.getUserByStudentId(studentId, connection);
            if (currentUser == null) {
            ...
            if (existingSession != null && existingSession.isActive()) {
                System.out.println("检测到用户已登录，强制注销旧会话: " + currentUser.getUsername());
                messageRouter.deregisterSession(existingSession.getUserId());
            }
            ...
            System.out.println("用户登录成功: " + currentUser.getUsername() + " (ID: " + currentUser.getId() + ")");
            ...
            try {
                ...
                if (systemRoom != null) {
                    // 检查用户是否已在system conversation中
                    if (systemRoom.getConversationId() != null) {
                        boolean userInSystemConv = conversationDAO.isConversationMember(systemRoom.getConversationId(), currentUser.getId(), connection);
            ...
            // 更新用户状态为ONLINE
            try {
                userDAO.updateUserStatus(currentUser.getId(), "ACTIVE", connection);
                System.out.println("用户状态已更新为ACTIVE: " + currentUser.getUsername());
            } catch (SQLException e) {
                System.err.println("更新用户状态失败: " + e.getMessage());
                e.printStackTrace();
            }
    ...
    private void handleUUIDAuth(Message message) {
        try (Connection connection = dbManager.getConnection()) {
            ...
            try {
                userDAO.updateUserStatus(currentUser.getId(), "ACTIVE", connection);
                System.out.println("用户状态已更新为ACTIVE: " + user.getUsername());
            } catch (SQLException e) {
                System.err.println("更新用户状态失败: " + e.getMessage());
                e.printStackTrace();
            }
    ...
    private void handleImageMessage(Message message) {
        ...
        if (conversationId != null) {
            try (Connection connection = dbManager.getConnection()) {
                ...
                // 根据会话类型处理消息
                if ("CLUB".equals(conversation.getType())) {
                    System.out.println("处理社团图片消息，会话ID: " + conversationId);
                    ...
                    // 保存社团图片消息到数据库
                    MessageDAO messageDAO = new MessageDAO();
                    messageDAO.saveMessage(imageMessage, "CLUB", conversationId, connection);
                    System.out.println("社团图片消息已保存到数据库");
    ...
    private void handleFileMessage(Message message) {
        ...
        if (conversationId != null) {
            try (Connection connection = dbManager.getConnection()) {
                ...
                // 根据会话类型处理消息
                if ("CLUB".equals(conversation.getType())) {
                    System.out.println("处理社团文件消息，会话ID: " + conversationId);
                    ...
                    // 保存社团文件消息到数据库
                    MessageDAO messageDAO = new MessageDAO();
                    messageDAO.saveMessage(fileMessage, "CLUB", conversationId, connection);
                    System.out.println("社团文件消息已保存到数据库");
                } else if ("FRIEND".equals(conversation.getType()) || "TEMP".equals(conversation.getType())) {
    ...
    private void sendUserInfo() {
        ...
        try {
            ...
            userInfoJson.append("{");
            userInfoJson.append("\"userId\":").append(currentUser.getId()).append(",");
            userInfoJson.append("\"username\":\"").append(currentUser.getUsername()).append("\"");
            if (avatarUrl != null && !avatarUrl.isEmpty()) {
                userInfoJson.append(",\"avatarUrl\":\"").append(avatarUrl).append("\"");
            }
            userInfoJson.append("}");
            
            Message userInfoMessage = new Message(
            ...
            send(messageCodec.encode(userInfoMessage));
            System.out.println("用户信息已发送到客户端: " + currentUser.getUsername() + ", avatarUrl: " + avatarUrl);
        } catch (Exception e) {
            System.err.println("发送用户信息失败: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void createAndRegisterSession() {
        ...
        if (!isConnected) {
            System.err.println("无法创建会话：WebSocket连接已关闭，用户ID=" + currentUser.getId() + ", 用户名=" + currentUser.getUsername());
            return;
        }
        ...
        if (currentSession != null) {
            ...
            if (registered) {
                // 查询用户的所有社团并自动加入
                try (Connection connection = dbManager.getConnection()) {
                    server.sql.club.ClubDAO clubDAO = new server.sql.club.ClubDAO(messageRouter);
                    List<Club> userClubs = clubDAO.getUserClubs(String.valueOf(currentUser.getId()), connection);
                    
                    for (Club club : userClubs) {
                        // 获取社团关联的conversation
                        Integer conversationId = club.getConversationId();
                        if (conversationId != null) {
                            // 检查用户是否已在conversation中
                            if (!conversationDAO.isConversationMember(conversationId, currentUser.getId(), connection)) {
                                conversationDAO.addConversationMember(conversationId, currentUser.getId(), "MEMBER", connection);
                                System.out.println("用户已加入社团conversation: " + currentUser.getUsername() + ", conversation_id: " + conversationId + ", club: " + club.getName());
                            }
                        }
                        System.out.println("用户已加入社团: " + club.getName() + " (ID: " + club.getId() + ")");
                    }
                } catch (SQLException e) {
                    // 如果表不存在，跳过社团加入逻辑
                    if (e.getMessage().contains("doesn't exist")) {
                        System.err.println("Club表不存在，跳过社团加入逻辑");
                    } else {
                        System.err.println("获取用户社团失败: " + e.getMessage());
                    }
                }
    ...
    private boolean isPrivateChat(String targetName) {
        ...
        // 检查是否是已知的房间名
        try (java.sql.Connection connection = dbManager.getConnection()) {
            return !clubDAO.clubExists(targetName, connection);
        } catch (java.sql.SQLException e) {
    ...
    private void handleRequestUserStats(Message message) {
        ...
        try (java.sql.Connection connection = dbManager.getConnection()) {
            ...
            // 获取社团数（从数据库查询）
            server.sql.club.ClubDAO clubDAO = new server.sql.club.ClubDAO(messageRouter);
            int clubCount = clubDAO.getUserClubCount(String.valueOf(currentUser.getId()), connection);
            ...
            // 发送响应
            send(messageCodec.encode(responseMsg));
            System.out.println("发送用户统计数据响应: " + username + " - 消息数: " + messageCount + ", 图片数: " + imageCount + ", 文件数: " + fileCount + ", 社团数: " + clubCount + ", 加入时间: " + joinTime + ", 状态: " + status);
        } catch (java.sql.SQLException e) {
    ...
    private void handleSearchRooms(Message message) {
        ...
        try (Connection connection = dbManager.getConnection()) {
            java.util.List<server.club.Club> rooms = clubDAO.searchClubs(searchTerm, connection);
            
            // 构建房间列表JSON
            com.google.gson.Gson gson = new com.google.gson.Gson();
            java.util.List<java.util.Map<String, Object>> roomList = new java.util.ArrayList<>();
            
            for (server.club.Club room : rooms) {
                ...
                // 将搜索到的社团添加到messageRouter（如果尚未存在）
                if (!messageRouter.getClubs().containsKey(room.getId())) {
                    // 查询conversation_id
                    String convSql = "SELECT id FROM conversation WHERE club_id = ? AND type = 'CLUB'";
                    try (PreparedStatement pstmt = connection.prepareStatement(convSql)) {
                        pstmt.setInt(1, Integer.parseInt(room.getId()));
                        try (ResultSet rs = pstmt.executeQuery()) {
                            if (rs.next()) {
                                room.setConversationId(rs.getInt("id"));
                            }
                        }
                    }
                    
                    // 查询owner_id
                    String ownerSql = "SELECT founder_id FROM club WHERE id = ?";
                    try (PreparedStatement pstmt = connection.prepareStatement(ownerSql)) {
                        pstmt.setInt(1, Integer.parseInt(room.getId()));
                        try (ResultSet rs = pstmt.executeQuery()) {
                            if (rs.next()) {
                                Integer founderId = rs.getInt("founder_id");
                                if (founderId != null && founderId > 0) {
                                    room.setOwnerId(String.valueOf(founderId));
                                }
                            }
                        }
                    }
                    ...
                    // 查询管理员ID
                    String adminSql = "SELECT user_id FROM club_member WHERE club_id = ? AND role = 'ADMIN'";
                    ...
                    messageRouter.addClub(room);
                    System.out.println("搜索时将社团添加到路由器: " + room.getName() + " (ID: " + room.getId() + ")");
                }
            }
    ...
    private void handleRequestRoomJoin(Message message) {
        ...
        if (roomId == null) {
            try (Connection connection = dbManager.getConnection()) {
                room = clubDAO.getClubByName(roomName, connection);
                if (room == null) {
                ...
                // 将社团添加到messageRouter
                messageRouter.addClub(room);
                System.out.println("从数据库加载社团并添加到路由器: " + roomName + " (ID: " + roomId + ")");
                
            } catch (SQLException e) {
                System.err.println("从数据库查询房间失败: " + e.getMessage());
                e.printStackTrace();
                Message errorMsg = new Message(MessageType.SYSTEM, "server", "查询房间失败");
                send(messageCodec.encode(errorMsg));
                return;
            }
        }
        ...
        // 检查用户是否已在房间中
        try (Connection connection = dbManager.getConnection()) {
            boolean alreadyInRoom = clubDAO.isUserInClub(roomId, String.valueOf(currentUser.getId()), connection);
            if (alreadyInRoom) {
        ...
        if (notifyUserIds.isEmpty()) {
            ...
            try (Connection connection = dbManager.getConnection()) {
                // 加入房间
                clubDAO.joinClub(roomId, String.valueOf(currentUser.getId()), connection);
                messageRouter.joinClub(String.valueOf(currentUser.getId()), roomId);
    ...
    private void handleRoomJoinRequest(Message message) {
        ...
        try (Connection connection = dbManager.getConnection()) {
            currentUserRole = clubDAO.getUserRole(roomId, currentUserId, connection);
        } catch (SQLException e) {
    ...
    private void handleRoomJoinResponse(Message message) {
        ...
        if ("accept".equalsIgnoreCase(response)) {
            ...
            try (Connection connection = dbManager.getConnection()) {
                ...
                // 检查用户是否已在房间中
                boolean alreadyInRoom = clubDAO.isUserInClub(roomId, requesterId, connection);
                
                if (!alreadyInRoom) {
                    // 加入房间
                    clubDAO.joinClub(roomId, requesterId, connection);
                    
                    // 如果用户在线，加入messageRouter
                    if (messageRouter.getSessions().containsKey(requesterId)) {
                        messageRouter.joinClub(requesterId, roomId);
                    }
    ...
    private void handleSetRoomAdmin(Message message) {
        ...
        try (Connection connection = dbManager.getConnection()) {
            // 获取房间ID
            Club room = clubDAO.getClubByName(roomName, connection);
            if (room == null) {
                Message errorMsg = new Message(MessageType.SYSTEM, "server", "房间 " + roomName + " 不存在", null);
                send(messageCodec.encode(errorMsg));
                return;
            ...
            // 检查当前用户是否为房主
            String currentUserRole = clubDAO.getUserRole(room.getId(), String.valueOf(currentUser.getId()), connection);
            ...
            // 检查目标用户是否在房间中
            if (!clubDAO.isUserInClub(room.getId(), targetUserId, connection)) {
            ...
            // 检查目标用户当前角色
            String targetUserRole = clubDAO.getUserRole(room.getId(), targetUserId, connection);
            ...
            // 更新用户角色为管理员
            boolean success = clubDAO.updateUserRole(room.getId(), targetUserId, "ADMIN", connection);
            if (success) {
    ...
    private void handleRemoveRoomAdmin(Message message) {
        ...
        try (Connection connection = dbManager.getConnection()) {
            ...
            // 更新用户角色为普通成员
            boolean success = clubDAO.updateUserRole(room.getId(), targetUserId, "MEMBER", connection);
            if (success) {
    ...
    private void handleUpdateRoomSettings(Message message) {
        ...
        try {
            ...
            // 获取房间ID
            Club room = clubDAO.getClubByName(roomName, dbManager.getConnection());
            if (room == null) {
                Message errorMsg = new Message(MessageType.SYSTEM, "server", "房间 " + roomName + " 不存在", null);
                send(messageCodec.encode(errorMsg));
                return;
            ...
            // 检查当前用户是否为房主
            String currentUserRole = clubDAO.getUserRole(room.getId(), String.valueOf(currentUser.getId()), dbManager.getConnection());
            if (!"OWNER".equals(currentUserRole)) {
            ...
            try (Connection connection = dbManager.getConnection()) {
                ...
                Boolean acceptTemporaryChat = (Boolean) settings.get("acceptTemporaryChat");
                if (acceptTemporaryChat != null) {
                    clubDAO.updateClubAcceptTemporaryChat(room.getId(), String.valueOf(currentUser.getId()), acceptTemporaryChat, connection);
                }
                
                // 更新房间类型
                String roomType = (String) settings.get("roomType");
                if (roomType != null && ("PUBLIC".equals(roomType) || "PRIVATE".equals(roomType))) {
                    clubDAO.updateClubType(room.getId(), roomType, connection);
                }
    ...
    private void handleRecallMessage(Message message) {
        ...
        try {
            ...
            if (deleted) {
                ...
                String roomId = null;
                for (String rId : messageRouter.getClubs().keySet()) {
                    if (roomName.equals(messageRouter.getClubs().get(rId).getName())) {
                        roomId = rId;
                        break;
                    }
                }
                ...
                if (roomId != null) {
                    ...
                    messageRouter.broadcastToClub(roomId, messageCodec.encode(successMsg));
    ...
    private boolean checkTemporaryChatPermission(String toUsername, Connection connection) throws SQLException {
        ...
        if (currentRoom != null && !currentRoom.equals("system")) {
            Club room = messageRouter.getClubs().get(currentRoom);
            if (room != null) {
                // 只有PUBLIC房间才允许临时聊天
                ...
                boolean acceptInRoom = clubDAO.isAcceptTemporaryChatInClub(room.getId(), toUserId, connection);
                if (!acceptInRoom) {
                    return false;
                }
            }
    ...
    private void handleUpdateAvatar(Message message) {
        try (Connection connection = dbManager.getConnection()) {
            String avatarUrl = message.getContent();
            
            if (avatarUrl == null || avatarUrl.isEmpty()) {
                Message errorMsg = new Message(MessageType.SYSTEM, "server", "头像URL不能为空");
                send(messageCodec.encode(errorMsg));
                return;
            }
            
            // 更新数据库中的头像URL
            boolean updated = userDAO.updateUserAvatarUrl(currentUser.getId(), avatarUrl, connection);
            
            if (updated) {
                System.out.println("用户头像已更新: " + currentUser.getUsername() + ", URL: " + avatarUrl);
                Message successMsg = new Message(MessageType.AVATAR_UPDATED, "server", avatarUrl, null);
                send(messageCodec.encode(successMsg));
            } else {
                Message errorMsg = new Message(MessageType.SYSTEM, "server", "更新头像失败");
                send(messageCodec.encode(errorMsg));
            }
            ...
            e.printStackTrace();
            Message errorMsg = new Message(MessageType.SYSTEM, "server", "更新头像失败: " + e.getMessage());
            send(messageCodec.encode(errorMsg));
        }
    }
}