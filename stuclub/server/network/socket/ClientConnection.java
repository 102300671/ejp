...
public class ClientConnection implements Runnable {
        ...
        try {
            switch (message.getType()) {
                    ...
                    if (content.startsWith("[room:")) {
                        ...
                        for (String rId : messageRouter.getClubs().keySet()) {
                            ...
                            if (room.getName().equals(roomName)) {
                                roomId = rId;
                                isPublicRoom = room instanceof PublicClub;
                                break;
                            }
                        }
                        ...
                        if (roomId != null) {
                            ...
                            // 广播消息
                            messageRouter.broadcastToClub(roomId, messageCodec.encode(broadcastMessage));
                            
                            // 保存房间消息到数据库
                            try (Connection connection = dbManager.getConnection()) {
                                MessageDAO messageDAO = new MessageDAO();
                                messageDAO.saveMessage(broadcastMessage, "CLUB", connection);
                            } catch (SQLException e) {
                ...
                case LEAVE:
                    ...
                    for (String leaveRoomId : messageRouter.getClubs().keySet()) {
                        if (actualRoomName.equals(messageRouter.getClubs().get(leaveRoomId).getName())) {
                            String userId = String.valueOf(currentUser.getId());
                ...
                case CREATE_ROOM:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        if (clubDAO.clubExists(createRoomName, connection)) {
                ...
                case EXIT_ROOM:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        String exitRoomName = "";
                        Club room = clubDAO.getClubByName(exitRoomName, connection);
                        if (room == null) {
                ...
                case LIST_ROOMS:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        String sql = "SELECT c.name, c.category FROM club c JOIN club_member cm ON c.id = cm.club_id WHERE cm.user_id = ? AND cm.status = 'APPROVED' AND c.status = 'APPROVED'";
                        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                            pstmt.setInt(1, currentUser.getId());
                            
                            try (ResultSet rs = pstmt.executeQuery()) {
                                StringBuilder roomsList = new StringBuilder("您所在的社团: ");
                                boolean first = true;
                                while (rs.next()) {
                                    if (!first) {
                                        roomsList.append(", ");
                ...
                case LIST_ROOM_USERS:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        List<Map<String, Object>> usersList = clubDAO.getClubMembersWithRoles(room.getId(), connection);
                        
                        // 获取在线用户ID集合
                        List<Map<String, Object>> roomUsersList = messageRouter.getClubUsers(room.getId());
                        ...
                        response.append("],\"currentUserRole\":\"").append(clubDAO.getUserRole(room.getId(), String.valueOf(currentUser.getId()), connection)).append("\"");
                ...
                case RECALL_MESSAGE:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        if (messageDAO.deleteMessage(messageId, connection)) {
                            ...
                            messageRouter.broadcastToClub(String.valueOf(recallConversationId), messageCodec.encode(recallMsg));
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        if ("display_name".equals(settingType)) {
                            ...
                            if (displayName != null && !displayName.trim().isEmpty()) {
                                if (!clubDAO.isClubDisplayNameAvailable(updateRoomId, displayName, connection)) {
                            ...
                            if (clubDAO.updateUserClubDisplayName(updateRoomId, String.valueOf(currentUser.getId()), displayName, connection)) {
                                ...
                                messageRouter.broadcastToClub(updateRoomId, messageCodec.encode(updateMsg));
                            } else {
                            ...
                            if (clubDAO.updateClubAcceptTemporaryChat(updateRoomId, String.valueOf(currentUser.getId()), accept, connection)) {
                ...
                case REQUEST_ROOM_DISPLAY_NAMES:
                    ...
                    try (Connection connection = dbManager.getConnection()) {
                        ...
                        List<java.util.Map<String, Object>> members = clubDAO.getClubMembersWithRoles(displayNamesRoomId, connection);
                        ...
                        for (java.util.Map<String, Object> member : members) {
                            ...
                            String displayName = clubDAO.getUserClubDisplayName(displayNamesRoomId, String.valueOf(userId), connection);
    ...
    private void handleLogin(Message message) {
        try (Connection connection = dbManager.getConnection()) {
            ...
            try {
                ...
                System.out.println("用户状态已更新为ACTIVE: " + username);
            } catch (SQLException e) {
    ...
    public synchronized void close() {
        ...
        if (isAuthenticated && currentUser != null && messageRouter != null) {
            ...
            // 更新用户状态为OFFLINE
            try (Connection connection = dbManager.getConnection()) {
                userDAO.updateUserStatus(currentUser.getId(), "INACTIVE", connection);
        ...
        if (currentSession != null) {
            ...
            if (registered) {
                ...
                for (String roomId : messageRouter.getClubs().keySet()) {
                    if ("system".equals(messageRouter.getClubs().get(roomId).getName())) {
                        // 加入system房间
                        messageRouter.joinClub(userId, roomId);
                        
                        // 将用户加入system conversation
                        Club systemRoom = messageRouter.getClubs().get(roomId);
