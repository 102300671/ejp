...
public class ConversationDAO {
    ...
    public int getOrCreateConversationFromOldRoom(int roomId, Connection connection) throws SQLException {
        String findSql = "SELECT c.id FROM conversation c " +
                         ...
                         "JOIN club_member rm ON cm.user_id = rm.user_id " +
                         "WHERE c.type = 'CLUB' AND rm.club_id = ? " +
                         "LIMIT 1";
        ...
        String roomSql = "SELECT name FROM club WHERE id = ?";
        ...
        if (roomName == null) {
            throw new SQLException("Club not found");
        }
        
        int conversationId = createConversation("CLUB", roomName, roomId, connection);
        
        String membersSql = "SELECT rm.user_id, rm.role FROM club_member rm " +
                           "WHERE rm.club_id = ?";
    ...
    public Conversation getConversationByRoomName(String roomName, Connection connection) throws SQLException {
        String sql = "SELECT id, type, name, created_at FROM conversation WHERE type = 'CLUB' AND name = ?";
        ...
        String oldRoomSql = "SELECT id FROM club WHERE name = ?";
    ...
    public Conversation getOrCreateRoomConversation(String roomName, Connection connection) throws SQLException {
        ...
        int conversationId = createConversation("CLUB", roomName, connection);
        return getConversation(conversationId, connection);
    }
    ...
    public List<Conversation> getAllPublicRoomConversations(Connection connection) throws SQLException {
        ...
        String sql = "SELECT id, type, name, created_at FROM conversation WHERE type = 'CLUB'";
        ...
        String oldRoomsSql = "SELECT id FROM club WHERE id NOT IN (" +
                             "SELECT rm.club_id FROM club_member rm " +
                             "JOIN conversation_member cm ON cm.user_id = rm.user_id " +
    ...
    public void performFullDataMigration(Connection connection) throws SQLException {
        String migrateRoomsSql = "SELECT id FROM club";
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    try {
                        ...
                        System.err.println("Failed to migrate club " + roomId + ": " + e.getMessage());
                    }
                }
            }
        }
    ...
    public void syncUserConversations(String username, Connection connection) throws SQLException {
        ...
        String syncRoomsSql = "SELECT r.id, r.name FROM club r " +
                              "JOIN club_member rm ON r.id = rm.club_id " +
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    try {
                        ...
                        System.err.println("Failed to sync club " + roomName + " for user " + username + ": " + e.getMessage());
                    }
                }
            }
        }
    ...
    public MigrationStatus checkMigrationStatus(Connection connection) throws SQLException {
        ...
        String oldRoomsSql = "SELECT COUNT(*) FROM club";
        try (PreparedStatement stmt = connection.prepareStatement(oldRoomsSql)) {
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
        ...
        String newRoomsSql = "SELECT COUNT(*) FROM conversation WHERE type = 'CLUB'";
    ...
    public List<Conversation> getAllClubConversations(Connection connection) throws SQLException {
        List<Conversation> conversations = new ArrayList<>();
        String sql = "SELECT id, type, name, club_id, created_at FROM conversation WHERE type = 'CLUB'";
        try (PreparedStatement stmt = connection.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                conv.setClubId(rs.getInt("club_id"));
