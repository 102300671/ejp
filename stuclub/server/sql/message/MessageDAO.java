...
public class MessageDAO {
    ...
    public List<Message> getConversationMessages(int conversationId, int limit, Connection connection) throws SQLException {
        String sql = "SELECT m.id, m.type, u.username as from_username, m.content, m.create_time FROM messages m " +
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                Message message = new Message(type, from, content, time, false, null, messageId, conversationId);
    ...
    public List<Message> getOfflineMessages(String username, String lastOnlineTime, Connection connection) throws SQLException {
        ...
        if (lastOnlineTime != null && !lastOnlineTime.isEmpty()) {
            ...
            sql = "SELECT m.id, m.type, u.username as from_username, m.content, m.create_time, m.conversation_id " +
                  "FROM messages m " +
                  "JOIN user u ON m.user_id = u.id " +
