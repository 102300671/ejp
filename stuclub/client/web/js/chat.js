...
let chatClient = {
    ...
    // 处理用户信息（包括头像URL）
    handleUserInfo: function(message) {
        this.log('info', '收到服务器用户信息');
        
        try {
            const userInfo = message.content ? JSON.parse(message.content) : {};
            
            if (userInfo.avatarUrl) {
                localStorage.setItem('userAvatarUrl', userInfo.avatarUrl);
                this.log('info', `用户头像URL: ${userInfo.avatarUrl}`);
                
                // 更新当前用户头像显示
                const avatarImg = document.querySelector('.user-avatar');
                if (avatarImg) {
                    this.loadUserAvatar(this.username, userInfo.avatarUrl);
                }
            }
        } catch (error) {
            this.log('error', `处理用户信息失败: ${error.message}`);
        }
    },
    
    // 处理头像更新成功
    handleAvatarUpdated: function(message) {
        this.log('info', '头像更新成功');
        
        try {
            const avatarUrl = message.content;
            if (avatarUrl) {
                localStorage.setItem('userAvatarUrl', avatarUrl);
                this.log('info', `头像已更新为: ${avatarUrl}`);
                
                // 更新头像显示
                const avatarImg = document.querySelector('.user-avatar');
                if (avatarImg) {
                    this.loadUserAvatar(this.username, avatarUrl);
                }
            }
        } catch (error) {
            this.log('error', `处理头像更新失败: ${error.message}`);
        }
    },
    ...
    sendMessage: function(type, to, content) {
        ...
        if (typeof type === 'object' && type !== null) {
            ...
            // LEAVE消息使用JSON格式，包含conversation_id和room_name
            if (type === MessageType.LEAVE && typeof to === 'string') {
                let conversationId = this.sessionToConversationId[to];
                if (conversationId) {
                    messageContent = JSON.stringify({
                        conversation_id: conversationId,
    ...
    handleMessage: function(message) {
        ...
        switch (message.type) {
            ...
            case MessageType.USER_INFO:
                ...
                break;
            case MessageType.AVATAR_UPDATED:
    ...
    switchChat: function(sessionId, roomType) {
        ...
        // Re-enable image and file upload buttons when switching to a public room
...
function initLogin() {
    ...
    document.getElementById('login-form').addEventListener('submit', function(e) {
        ...
        const sendLogin = setInterval(() => {
            if (chatClient.isConnected) {
                ...
                chatClient.sendMessage(MessageType.LOGIN, 'server', studentId + ':' + password);
            }
        }, 100);
        ...
        setTimeout(() => {
            clearInterval(sendLogin);
            if (!chatClient.isConnected) {
                document.getElementById('message').textContent = '连接失败，请重试';
            }
        }, 5000);
    });
    ...
    document.getElementById('register-form').addEventListener('submit', function(e) {
        ...
        if (!role) {
            document.getElementById('message').textContent = '请选择身份';
            return;
        }
        
        if (!realName) {
            document.getElementById('message').textContent = '请填写真实姓名';
            return;
        }
        
        if (!studentId) {
            document.getElementById('message').textContent = role === 'TEACHER' ? '请填写工号' : '请填写学号';
            return;
        }
        
        if (password !== confirmPassword) {
            document.getElementById('message').textContent = '密码不匹配';
            return;
        }
        ...
        const sendRegister = setInterval(() => {
            if (chatClient.isConnected) {
                ...
                const registerContent = username + ':' + password + ':' + role + ':' + realName + ':' + studentId;
                chatClient.sendMessage(MessageType.REGISTER, 'server', registerContent);
            }
        }, 100);
                ...
                .then(data => {
                    if (data && data.code === '0') {
                        ...
                        const updateMessage = {
                            ...
                            id: `UPDATE_AVATAR_${username}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
                        };
                        
                        if (chatClient.ws && chatClient.ws.readyState === WebSocket.OPEN) {
                            chatClient.ws.send(JSON.stringify(updateMessage));
                            console.log('Sent UPDATE_AVATAR message to server');
                        }
...
function loadUserAvatar(imgElement, username, savedAvatarUrl) {
    // 如果有保存的头像URL（来自数据库），直接使用
    if (savedAvatarUrl) {
        ...
        imgElement.onerror = function() {
            // 如果头像加载失败，显示默认头像
            showDefaultAvatar(imgElement, username);
        };
        return;
    }
...
// 显示默认头像
function showDefaultAvatar(imgElement, username) {
    const firstLetter = username ? username.charAt(0).toUpperCase() : '?';
    const colors = ['%234a6fa5', '%235a9fd4', '%236b8e23', '%23cd853f', '%238b8b00', '%23cd5c5c', '%23ba55d3'];
    const colorIndex = username ? username.charCodeAt(0) % colors.length : 0;
    const defaultAvatar = `data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"%3E%3Ccircle cx="50" cy="50" r="45" fill="${colors[colorIndex]}"/%3E%3Ctext x="50" y="60" font-size="40" text-anchor="middle" fill="white"%3E${firstLetter}%3C/text%3E%3C/svg%3E`;
...
function initChat() {
    ...
    if (chatsPanel && messagesPanel && window.innerWidth <= 768) {
        chatsPanel.classList.add('hidden');
        messagesPanel.classList.add('active');
        // Hide activities panel on mobile by default
        if (activitiesPanel) {
            activitiesPanel.classList.remove('active');
        }
    }
    
    // Add activities panel toggle button for mobile
    if (window.innerWidth <= 768 && activitiesPanel) {
        const headerControls = document.querySelector('.chat-header .chat-controls');
        if (headerControls) {
            const activitiesBtn = document.createElement('button');
            activitiesBtn.id = 'toggle-activities-btn';
            activitiesBtn.textContent = '活动';
            activitiesBtn.style.cssText = 'background-color: #17a2b8; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px;';
            activitiesBtn.addEventListener('click', function() {
                activitiesPanel.classList.toggle('active');
                messagesPanel.classList.toggle('active', !activitiesPanel.classList.contains('active'));
                chatsPanel.classList.add('hidden');
            });
            headerControls.appendChild(activitiesBtn);
        }
    }
    ...
    document.getElementById('leave-room-btn').addEventListener('click', function() {
        const leavingClub = chatClient.currentChat;
        if (!leavingClub) {
            return;
        }
        
        chatClient.sendMessage(MessageType.LEAVE, leavingClub, chatClient.username + ' 退出了社团');
        chatClient.currentChat = null;
...
function initUserMenu() {
    ...
    if (userAvatar) {
        ...
        if (username) {
            ...
            loadUserAvatar(userAvatar, username, savedAvatarUrl);
        }
    }
    ...
    if (userMenuBtn && userMenuDropdown) {
        ...
        // Load activities (default tab)
        loadActivities();
        
        // Tab switching
        document.getElementById('tab-activities').addEventListener('click', function() {
        ...
        document.getElementById('refresh-activities-btn').addEventListener('click', function() {
            const activeTab = document.querySelector('.activities-panel .tab-btn.active');
            if (activeTab.id === 'tab-activities') {
                loadActivities();
            } else {
                loadAnnouncements();
            }
    ...
    // Switch between activities and announcements tab
    function switchTab(tab) {
        ...
        if (tab === 'activities') {
            ...
            if (activitiesList.innerHTML === '<div class="loading">加载中...</div>' || 
                activitiesList.innerHTML === '<div class="empty">暂无活动公告</div>' ||
                activitiesList.innerHTML === '<div class="empty">暂无活动和公告</div>') {
                loadActivities();
            }
        } else {
            ...
            if (announcementsList.innerHTML === '<div class="loading">加载中...</div>') {
                loadAnnouncements();
            }
        }
    }
    ...
    function loadActivities() {
        ...
        fetch('http://localhost:8086/api/activities')
            .then(response => {
            ...
            .then(activities => {
                if (!activities || activities.length === 0) {
                    activitiesList.innerHTML = '<div class="empty">暂无活动</div>';
                    return;
                }
                
                activitiesList.innerHTML = activities.slice(0, 10).map(activity => {
                    const startDate = activity.start_time || activity.startDate || '';
                    const endDate = activity.end_time || activity.endDate || '';
                    const clubName = activity.club_name || activity.clubName || '';
                    ...
                    return `
                        <div class="activity-item" onclick="showActivityDetail(${activity.id})">
                            ...
                            <div class="activity-info">
                                ${clubName ? `<span class="activity-club">${escapeHtml(clubName)}</span>` : ''}
                                <span class="activity-date">${formatDate(startDate)}</span>
                                ${description ? `<span>${escapeHtml(description.substring(0, 50))}${description.length > 50 ? '...' : ''}</span>` : ''}
                                <span class="activity-status ${status}">${getStatusText(status)}</span>
                            </div>
                        </div>
                    `;
                }).join('');
            ...
            .catch(error => {
                ...
                activitiesList.innerHTML = '<div class="error">加载失败，请刷新重试</div>';
            });
    }
    
    // Load announcements from dashboard API
    function loadAnnouncements() {
        const announcementsList = document.getElementById('announcements-list');
        if (!announcementsList) return;
        
        announcementsList.innerHTML = '<div class="loading">加载中...</div>';
        
        fetch('http://localhost:8086/api/announcements')
            .then(response => {
                if (!response.ok) throw new Error('Network response was not ok');
                return response.json();
            })
            .then(announcements => {
                if (!announcements || announcements.length === 0) {
                    announcementsList.innerHTML = '<div class="empty">暂无公告</div>';
                    return;
                }
                
                announcementsList.innerHTML = announcements.slice(0, 10).map(announcement => {
                    const title = announcement.title || '';
                    const content = announcement.content || '';
                    const clubName = announcement.club_name || announcement.clubName || '';
                    const priority = announcement.priority || 'NORMAL';
                    const publishedAt = announcement.published_at || announcement.created_at || '';
                    
                    return `
                        <div class="activity-item announcement-item" onclick="showAnnouncementDetail(${announcement.id})">
                            <div class="activity-title">${announcement.is_pinned ? '📌 ' : ''}${escapeHtml(title)}</div>
                            <div class="activity-info">
                                ${clubName ? `<span class="activity-club">${escapeHtml(clubName)}</span>` : ''}
                                <span class="activity-date">${formatDate(publishedAt)}</span>
                                ${content ? `<span>${escapeHtml(content.substring(0, 50))}${content.length > 50 ? '...' : ''}</span>` : ''}
                                <span class="activity-status priority-${priority.toLowerCase()}">${getPriorityText(priority)}</span>
                            </div>
                        </div>
                    `;
                }).join('');
            ...
            .catch(error => {
                ...
                announcementsList.innerHTML = '<div class="error">加载失败，请刷新重试</div>';
            });
    }
    ...
    // Show activity detail (open in new tab or modal)
    function showActivityDetail(activityId) {
        window.open(`http://localhost:8086/activities/${activityId}`, '_blank');
    }
    
    // Show announcement detail (open in new tab or modal)
    function showAnnouncementDetail(announcementId) {
        window.open(`http://localhost:8086/announcements/${announcementId}`, '_blank');
    }
}

// Activity and Announcement Global Functions
function showActivityDetail(activityId) {
    fetch(`http://localhost:8086/api/activities/${activityId}`)
        .then(response => {
        ...
        .then(activity => {
            ...
            const content = `
                ...
                <p><strong>人数：</strong>${activity.current_participants || 0}/${activity.max_participants || 0}</p>
            `;
...
function showAnnouncementDetail(announcementId) {
    fetch(`http://localhost:8086/api/announcements/${announcementId}`)
        .then(response => {
            if (!response.ok) throw new Error('加载失败');
            return response.json();
        })
        .then(announcement => {
            ...
            const content = `
                <h3>${announcement.is_pinned ? '📌 ' : ''}${announcement.title}</h3>
                <p><strong>发布社团：</strong>${clubName || '系统'}</p>
                <p><strong>优先级：</strong>${priorityText}</p>
                <p><strong>发布时间：</strong>${formatDate(publishedAt)}</p>
                ...
                if (username) {
                    uploadAvatarToServer(username, file, function(success, result) {
                        if (success) {
                            ...
                            const savedAvatarUrl = localStorage.getItem('userAvatarUrl');
                            loadUserAvatar(document.getElementById('profile-avatar'), username, savedAvatarUrl);
                        } else {
