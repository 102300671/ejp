...
public class UserController {
    ...
    public ResponseEntity<String> createUser(@RequestBody Map<String, String> request) {
        ...
        boolean success = userService.createUser(username, password, realName, studentId, phone, email, role);
        if (success) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<String> updateUser(@PathVariable int id, @RequestBody Map<String, String> request) {
        String realName = request.get("realName");
        String studentId = request.get("studentId");
        String phone = request.get("phone");
        String email = request.get("email");
        ...
        if (role == null || status == null) {
            return ResponseEntity.badRequest().body("角色和状态不能为空");
        }
        
        boolean success = userService.updateUser(id, realName, studentId, phone, email, role, status);
        if (success) {
