...
public class MessageService {
    ...
    public List<Map<String, Object>> getAllMessages(int limit, int offset) {
        String sql = "SELECT m.id, m.type, u.username as from_username, m.conversation_id, m.content, m.create_time, " +
    ...
    public List<Map<String, Object>> getPrivateMessages(String user1, String user2, int limit) {
        String sql = "SELECT m.id, m.type, u.username as from_username, m.conversation_id, m.content, m.create_time " +
                     ...
                     "JOIN user u1 ON cm1.user_id = u1.id " +
                     "JOIN user u2 ON cm2.user_id = u2.id " +
    ...
    public Map<String, Object> getMessageStats() {
        ...
        stats.put("roomMessages", jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM messages m JOIN conversation c ON m.conversation_id = c.id WHERE c.type = 'CLUB'", Integer.class));
        stats.put("privateMessages", jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM messages m JOIN conversation c ON m.conversation_id = c.id WHERE c.type = 'PRIVATE'", Integer.class));
        stats.put("nsfwMessages", 0);
        return stats;
    }
    
    public List<Map<String, Object>> getTopUsersByMessageCount(int limit) {
        String sql = "SELECT u.username as from_username, COUNT(*) as message_count " +
                     "FROM messages m " +
                     "LEFT JOIN user u ON m.user_id = u.id " +
                     "GROUP BY u.username " +
