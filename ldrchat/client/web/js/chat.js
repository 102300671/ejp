...
const MessageType = {
    ...
    SEARCH_CP: 'SEARCH_CP',
    SEND_CP_REQUEST: 'SEND_CP_REQUEST',
    ACCEPT_CP_REQUEST: 'ACCEPT_CP_REQUEST',
    REJECT_CP_REQUEST: 'REJECT_CP_REQUEST',
    UNBIND_CP: 'UNBIND_CP',
    CHECK_CP_STATUS: 'CHECK_CP_STATUS'
};
...
let chatClient = {
    ...
    handleAuthSuccess: function(message) {
        ...
        this.showToast('登录成功！正在跳转到主页面...', 'success');
    ...
    handleSystemMessage: function(message) {
        ...
        // CP绑定相关消息处理
        // 搜索CP结果
        if (message.content.includes('找到用户: ')) {
            const searchResults = document.getElementById('cp-search-results');
            const username = message.content.replace('找到用户: ', '');
            searchResults.innerHTML = `
                <div class="cp-search-result-item">
                    <span>${username}</span>
                    <button class="send-cp-request-btn" onclick="sendCPRequest('${username}')">💕 发送请求</button>
                </div>
            `;
            return;
        }
        ...
        // CP绑定成功
        if (message.content.includes('CP绑定成功')) {
            // 提取会话ID
            const match = message.content.match(/会话ID:\s*(\d+)/);
            if (match && match[1]) {
                sessionStorage.setItem('currentConversationId', match[1]);
            }
            
            // 从消息内容中提取CP用户名（需要服务器端支持）
            // 这里暂时使用模拟方式，实际应该从服务器响应中获取
            const cpUsername = message.content.includes('接受了您的CP绑定请求') ? 
                message.content.substring(0, message.content.indexOf(' 接受了您的CP绑定请求')) : 
                '我的CP';
            
            localStorage.setItem('cpUsername', cpUsername);
            localStorage.setItem('loveStartDate', new Date().toISOString().split('T')[0]);
            
            // 显示成功提示并重新初始化
            this.showToast('💕 CP绑定成功！', 'success');
            setTimeout(() => {
                initCPBinding();
            }, 1000);
            return;
        ...
        // 收到CP绑定请求
        if (message.content.includes('向您发送了CP绑定请求')) {
            const requester = message.content.substring(0, message.content.indexOf(' 向您发送了CP绑定请求'));
            currentCPRequester = requester;
            document.getElementById('request-sender').textContent = requester;
            document.getElementById('cp-request-received').style.display = 'block';
            document.getElementById('bind-cp-btn').style.display = 'none';
            document.getElementById('cp-request-pending').style.display = 'none';
            this.showToast(`💕 ${requester} 向您发送了CP绑定请求！`, 'info');
            return;
        }
        
        // CP绑定请求被接受
        if (message.content.includes('接受了您的CP绑定请求')) {
            const accepter = message.content.substring(0, message.content.indexOf(' 接受了您的CP绑定请求'));
            ...
            localStorage.setItem('cpUsername', accepter);
            localStorage.setItem('loveStartDate', new Date().toISOString().split('T')[0]);
            
            // 隐藏等待状态并重新初始化
            document.getElementById('cp-request-pending').style.display = 'none';
            this.showToast(`💕 ${accepter} 接受了您的CP绑定请求！`, 'success');
            setTimeout(() => {
                initCPBinding();
            }, 1000);
            return;
        ...
        // CP绑定请求被拒绝
        if (message.content.includes('拒绝了您的CP绑定请求')) {
            const rejecter = message.content.substring(0, message.content.indexOf(' 拒绝了您的CP绑定请求'));
            document.getElementById('cp-request-pending').style.display = 'none';
            document.getElementById('bind-cp-btn').style.display = 'block';
            this.showToast(`💔 ${rejecter} 拒绝了您的CP绑定请求`, 'error');
            return;
        }
        
        // CP解绑
        if (message.content.includes('已解除CP绑定')) {
            localStorage.removeItem('cpUsername');
            localStorage.removeItem('loveStartDate');
            this.showToast('💔 CP绑定已解除', 'info');
            setTimeout(() => {
                initCPBinding();
            }, 1000);
            return;
        ...
        // CP状态检查结果
        if (message.content.includes('已绑定CP:')) {
            const cpUsername = message.content.replace('已绑定CP: ', '');
            localStorage.setItem('cpUsername', cpUsername);
            initCPBinding();
            return;
        }
        
        if (message.content.includes('已发送绑定请求')) {
            document.getElementById('bind-cp-btn').style.display = 'none';
            document.getElementById('cp-request-pending').style.display = 'block';
            return;
        }
        
        if (message.content.includes('收到来自 ') && message.content.includes(' 的绑定请求')) {
            const requester = message.content.replace('收到来自 ', '').replace(' 的绑定请求', '');
            currentCPRequester = requester;
            document.getElementById('request-sender').textContent = requester;
            document.getElementById('cp-request-received').style.display = 'block';
            document.getElementById('bind-cp-btn').style.display = 'none';
        ...
        // 注册成功后自动跳转到登录页面
        if (message.content.includes('注册成功')) {
            const messageElement = document.getElementById('message');
            if (messageElement) {
    ...
    document.getElementById('login-form').addEventListener('submit', function(e) {
        ...
        const sendLogin = setInterval(() => {
            if (chatClient.isConnected) {
                ...
                chatClient.sendMessage(MessageType.LOGIN, 'server', JSON.stringify({username: username, password: password}));
            }
        }, 100);
    ...
    document.getElementById('register-form').addEventListener('submit', function(e) {
        ...
        const sendRegister = setInterval(() => {
            if (chatClient.isConnected) {
                ...
                chatClient.sendMessage(MessageType.REGISTER, 'server', JSON.stringify({username: username, password: password}));
            }
        }, 100);
...
// 初始化CP绑定相关功能
function initCPBinding() {
    const coupleBindPanel = document.getElementById('couple-bind-panel');
    const coupleChatPanel = document.getElementById('couple-chat-panel');
    const cpUsername = localStorage.getItem('cpUsername');
    
    // 检查是否已绑定CP
    if (cpUsername) {
        // 已绑定，显示聊天面板
        coupleBindPanel.style.display = 'none';
        coupleChatPanel.style.display = 'flex';
        document.getElementById('current-chat-name').textContent = `💑 ${cpUsername}`;
    } else {
        // 未绑定，显示绑定面板
        coupleBindPanel.style.display = 'flex';
        coupleChatPanel.style.display = 'none';
        checkCPRequest();
    }
...
// 绑定CP相关事件监听器
function bindCPListeners() {
    // 绑定CP按钮
    document.getElementById('bind-cp-btn').addEventListener('click', function() {
        document.getElementById('cp-search-modal').style.display = 'block';
    });
    
    // 搜索CP按钮
    document.getElementById('search-cp-btn').addEventListener('click', searchCP);
    
    // CP搜索输入框回车事件
    document.getElementById('cp-search-input').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            searchCP();
        }
    });
    ...
    // 解绑CP按钮
    document.getElementById('unbind-cp-btn').addEventListener('click', function() {
        document.getElementById('unbind-confirm-modal').style.display = 'block';
    });
    
    // 确认解绑按钮
    document.getElementById('confirm-unbind-btn').addEventListener('click', unbindCP);
    
    // 取消解绑按钮
    document.getElementById('cancel-unbind-btn').addEventListener('click', function() {
        document.getElementById('unbind-confirm-modal').style.display = 'none';
    });
    
    // 接受CP请求按钮
    document.getElementById('accept-cp-btn').addEventListener('click', acceptCPRequest);
    
    // 拒绝CP请求按钮
    document.getElementById('reject-cp-btn').addEventListener('click', rejectCPRequest);
...
function searchCP() {
    ...
    if (!searchTerm) {
        searchResults.innerHTML = '<p style="text-align: center; color: #666;">请输入用户名</p>';
        return;
    }
    
    // 使用WebSocket发送搜索请求
    const message = {
        type: MessageType.SEARCH_CP,
        content: searchTerm,
        time: Date.now()
    };
    ...
    // 等待服务器响应，响应会在onMessage中处理
    searchResults.innerHTML = '<p style="text-align: center; color: #666;">搜索中...</p>';
}

// 发送CP绑定请求
function sendCPRequest(targetUsername) {
    const currentUsername = sessionStorage.getItem('username') || localStorage.getItem('username');
    
    // 使用WebSocket发送绑定请求
    const message = {
        type: MessageType.SEND_CP_REQUEST,
    ...
    // 显示等待确认状态
    document.getElementById('cp-search-modal').style.display = 'none';
    document.getElementById('bind-cp-btn').style.display = 'none';
    document.getElementById('cp-request-pending').style.display = 'block';
    document.getElementById('cp-request-received').style.display = 'none';
}

// 检查是否有CP请求
function checkCPRequest() {
    // 使用WebSocket检查CP状态
    const message = {
        type: MessageType.CHECK_CP_STATUS,
        content: '',
        time: Date.now()
    };
...
// 全局变量：存储当前收到的CP请求发送者
let currentCPRequester = null;

// 接受CP请求
function acceptCPRequest() {
    if (!currentCPRequester) {
        console.error('没有待处理的CP请求');
        return;
    }
    
    // 使用WebSocket发送接受请求
    const message = {
        type: MessageType.ACCEPT_CP_REQUEST,
        content: currentCPRequester,
        time: Date.now()
    };
    ...
    document.getElementById('cp-request-received').style.display = 'none';
    document.getElementById('bind-cp-btn').style.display = 'block';
    currentCPRequester = null;
}

// 拒绝CP请求
function rejectCPRequest() {
    if (!currentCPRequester) {
        console.error('没有待处理的CP请求');
        return;
    }
    ...
    // 使用WebSocket发送拒绝请求
    const message = {
        type: MessageType.REJECT_CP_REQUEST,
        content: currentCPRequester,
        time: Date.now()
    };
...
function updateLoveDays() {
    // 获取恋爱开始日期（从本地存储读取）
    let loveStartDate = localStorage.getItem('loveStartDate');
    const cpUsername = localStorage.getItem('cpUsername');
    
    const loveDaysElement = document.getElementById('love-days');
    
    // 如果没有设置恋爱开始日期且已绑定CP
    if (!loveStartDate && cpUsername) {
        // 从绑定CP开始计算，默认设置为今天
        loveStartDate = new Date().toISOString().split('T')[0];
        localStorage.setItem('loveStartDate', loveStartDate);
    }
    
    // 如果没有恋爱开始日期（未绑定CP或刚注册）
    if (!loveStartDate) {
        if (loveDaysElement) {
            loveDaysElement.textContent = '-';
        }
        return;
    }
    ...
    if (loveDaysElement) {
        loveDaysElement.textContent = `${days} 天`;
        // 添加点击事件以允许修改日期
        loveDaysElement.style.cursor = 'pointer';
        loveDaysElement.title = '点击设置恋爱开始日期';
        loveDaysElement.addEventListener('click', openLoveDateSettings);
    }
}

// 打开恋爱日期设置弹窗
function openLoveDateSettings() {
    // 创建弹窗
    let modal = document.getElementById('love-date-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'love-date-modal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content">
                <span class="close" onclick="closeLoveDateModal()">&times;</span>
                <h3>💕 设置恋爱开始日期</h3>
                <div class="form-group">
                    <label for="love-start-date">恋爱开始日期:</label>
                    <input type="date" id="love-start-date" max="${new Date().toISOString().split('T')[0]}">
                </div>
                <div class="form-group">
                    <button id="save-love-date-btn" class="action-btn primary">保存</button>
                    <button id="cancel-love-date-btn" class="action-btn secondary">取消</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
        ...
        // 添加事件监听
        document.getElementById('save-love-date-btn').addEventListener('click', saveLoveDate);
        document.getElementById('cancel-love-date-btn').addEventListener('click', closeLoveDateModal);
    ...
    // 设置当前日期值
    const currentDate = localStorage.getItem('loveStartDate');
    if (currentDate) {
        document.getElementById('love-start-date').value = currentDate;
    }
...
function closeLoveDateModal() {
    const modal = document.getElementById('love-date-modal');
    if (modal) {
        modal.style.display = 'none';
    }
}
...
function saveLoveDate() {
    ...
    if (selectedDate) {
        localStorage.setItem('loveStartDate', selectedDate);
        updateLoveDays();
        closeLoveDateModal();
        
        // 显示成功提示
        chatClient.showToast('💕 恋爱开始日期已更新！', 'success');
    } else {
        chatClient.showToast('请选择一个日期', 'error');
    }
}
...
function updateDistance() {
    // 模拟距离（可以从本地存储读取实际距离）
    let distance = localStorage.getItem('coupleDistance');
    if (!distance) {
        // 默认距离为1000公里
        distance = '1000';
        localStorage.setItem('coupleDistance', distance);
    }
...
function initChatUI() {
    ...
    if (privateMsgBtn) {
        privateMsgBtn.addEventListener('click', () => {
            ...
            if (membersButton && membersButton.disabled) {
                return;
            }
    ...
    const joinChatCloseBtn = joinChatModal ? joinChatModal.querySelector('.close') : null;
    ...
    const imageModalCloseBtn = imageModal ? imageModal.querySelector('.close') : null;
    const modalNsfwToggleBtn = document.getElementById('modal-nsfw-toggle-btn');
    
    // Image upload modal functionality
    const imageUploadModal = document.getElementById('image-upload-modal');
    const imageUploadCloseBtn = imageUploadModal ? imageUploadModal.querySelector('.close') : null;
    
    // Close image upload modal when close button is clicked
    if (imageUploadCloseBtn) {
    ...
    if (nsfwCheckbox) {
        nsfwCheckbox.addEventListener('change', function() {
            ...
            if (warningDiv) {
                warningDiv.style.display = this.checked ? 'flex' : 'none';
            }
        });
    }
    
    // Modal NSFW toggle button functionality
    if (modalNsfwToggleBtn) {
...
window.openFileModal = async function(fileUrl, fileName, fileType, isNSFW = false, openMode = 'edit') {
    ...
    if (leaveRoomBtn) {
        leaveRoomBtn.addEventListener('click', function() {
            ...
            if (currentChatName) {
                currentChatName.textContent = 'System Chat';
            }
